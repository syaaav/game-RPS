  ...
  if (data.status === "error") {
    if (data.message === "no game id") {
      ...
      document.querySelector(".errorText").textContent =
        "Кажется, Id этой игры не был передан.";
    } else if (data.message === "wrong game id") {
      window.application.renderScreen("error");
      document.querySelector(".errorText").textContent =
        "Бой уже закончен. Кажется, ваш соперник испугался и убежал!";
    } else if (data.message === "not your move") {
      document.querySelector(".namesOpponents").textContent =
        "Вы уже отправили ход в текущем раунде. Давайте подождём ход вашего соперника?";
    } else if (data.message === "wrong move") {
      document.querySelector(".namesOpponents").textContent =
        "Недопустимый ход! Попробуйте что-нибудь другое ;)";
    } else if (data.message === "no move") {
      document.querySelector(".namesOpponents").textContent =
        "Ход не передан! Попробуйте ещё раз ;)";
    } else if (data.message === "token doesn't exist") {
      window.application.renderScreen("error");
      document.querySelector(".errorText").textContent =
        " ̶Н̶е̶т̶ ̶и̶г̶р̶о̶к̶а̶ ̶с̶ ̶т̶а̶к̶и̶м̶ ̶т̶о̶к̶е̶н̶о̶м̶.̶ Вас не существует!";
    } else if (data.message === "player is not in this game") {
      window.application.renderScreen("error");
      document.querySelector(".errorText").textContent =
        "Такой игрок уже в игре! Вернитесь в активную игру или введите новый никнейм ;)";
    }
